...
| `repository`          | URL    | Specify the [package repository URL][repository] to be used as the main repository. Currently only http(s) repositories are supported.                                                                                                |
...
| `version`             | String | <p>TeX Live version to install. Supported values are `2008` to `2024`, and `latest`.</p> **Default:**&ensp;`latest` if `repository` input is not set, otherwise the remote version will be assumed.                                   |
